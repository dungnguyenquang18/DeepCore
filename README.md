...
1. Mở terminal và điều hướng đến thư mục chứa `main.py`.
2. Chạy lệnh sau để bắt đầu quá trình huấn luyện:

```bash
python main.py --dataset CIFAR10 --model ResNet18 --epochs 200 --data_path ./data --save_path ./results
```

Bạn có thể tùy chỉnh các tham số khác theo nhu cầu của mình.

### Cách 2: Sử dụng `demo.ipynb`

1. Mở Jupyter Notebook và điều hướng đến file `demo.ipynb`.
2. Chạy từng cell trong notebook để thực hiện các bước tải dữ liệu, huấn luyện và kiểm tra mô hình.
