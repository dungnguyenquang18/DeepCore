...
class Forgetting(EarlyTrain):
    ...
    def after_loss(self, outputs, loss, targets, batch_inds, epoch):
        with torch.no_grad():
            ...
            index_tensor = torch.tensor(batch_inds, device=self.forgetting_events.device)[(self.last_acc[batch_inds] - cur_acc) > 0.01]

            self.forgetting_events[index_tensor] += 1.
    ...
    def select(self, **kwargs):
        ...
        if not self.balance:
            ...
            for c in range(self.num_classes):
                c_indx = self.train_indx[self.dst_train.y == c]
                # c_indx = self.train_indx[torch.argmax(self.dst_train.y, dim=1) == c]
